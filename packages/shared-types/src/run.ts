...
export const runSchema = z.object({
  ...
  // CLI-generated fix-prompt (BYOK Flavor #1: LLM runs CLI-side, cloud holds
  // no key). Null on --no-fix-prompt opt-out, LLM endpoint failure, or legacy
  // pre-M4-1 runs.
  fix_prompt: z.string().nullable().default(null),
  // S3 URL of the raw events.jsonl blob (V1.5 re-correlate uses this).
  // Null in self-host mode, --no-upload opt-out, or legacy pre-M4-1 runs.
  // Per FDRS-327 "events_jsonl_url text NULL" — drift with FDRS-318
  // description ("string") resolved nullable here.
  events_jsonl_url: z.string().url().nullable().default(null),
