...
in-process; the Slack and Stripe twins are vendored into `cli/src/twin-{slack,stripe}/`
(mirroring `cli/src/twin-github/`); `slackSeedStateSchema` joins the seed union;
new deterministic `slack` twin-plugin; the Stripe twin gains the JSON-RPC `/mcp`
endpoint the mcp-loop fleet needs plus `create_refund`/`retrieve_refund`/`list_refunds`
tools, and the `stripe` plugin gains an event-based refund-attempt `[D]`.
